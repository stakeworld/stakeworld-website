...
Last update: Sat 25 Nov

|  | Chain    | Database   | Format | Snapshot |  
| ------------------------| ----------- | -------- | ------- | ----------- | 
| [direct link](http://snapshot.stakeworld.io/paritydb-ksmcc3.lz4) | ksmcc3 | paritydb | pruned | 333G |
| [direct link](http://snapshot.stakeworld.io/paritydb-polkadot.lz4) | polkadot | paritydb | pruned | 267G |
...
For now the snapshots are available for kusama `--chain ksmcc3` and polkadot `--chain polkadot` in the paritydb `--database paritydb` database format. They are pruned with `--state-pruning 256` (default), which is sufficient for a validator node.
:::
