...
| ------------------------| ----------- | -------- | ------- | ----------- | ---------- | ------------ | -------- |
| [direct link](http://snapshot.stakeworld.io/rocksdb-ksmcc3.lz4) | ksmcc3 | rocksdb | pruned | 15979360 | 201G | 214G | Sat 31 Dec @ 02:22 |
| [direct link](http://snapshot.stakeworld.io/paritydb-ksmcc3.lz4) | ksmcc3 | paritydb | pruned | 15979471 | 156G | 163G | Sat 31 Dec @ 02:33 |
| [direct link](http://snapshot.stakeworld.io/rocksdb-polkadot.lz4) | polkadot | rocksdb | pruned | 13590173 | 110G | 115G | Sat 31 Dec @ 02:40 |
| [direct link](http://snapshot.stakeworld.io/paritydb-polkadot.lz4) | polkadot | paritydb | pruned | 13590244 | 111G | 115G | Sat 31 Dec @ 02:47 |
| | ksmcc3 | paritydb | archive | 15979673 | | 1.2T | Sat 31 Dec @ 02:53 |
| | polkadot | paritydb | archive | 13590301 | | 752G | Sat 31 Dec @ 02:53 |
...
**DBDIR**: By default the polkadot binary runs in `/root/.local/share/polkadot/chains/<chain>` but it is easier and less error sensitive to run it in `/home/polkadot/<mynodename>/chains/<chain>` and set the datadir in your startup options, for example `--base-path /home/polkadot/mynode`

So for example to restore a kusama paritydb database snapshot for your node with `--base-path /home/polkadot/mynode`:

```bash
mkdir -p /home/polkadot/mynode/chains/ksmcc3
...
### Paritydb kusama
```bash
curl -o - -L http://snapshot.stakeworld.io/paritydb-ksmcc3.lz4 | lz4 -c -d - | tar -x -C DBDIR
```

### Paritydb polkadot
```bash
curl -o - -L http://snapshot.stakeworld.io/paritydb-polkadot.lz4 | lz4 -c -d - | tar -x -C DBDIR
```

### Rocksdb kusama
```bash
curl -o - -L http://snapshot.stakeworld.io/rocksdb-ksmcc3.lz4 | lz4 -c -d - | tar -x -C DBDIR
```

### Rocksdb polkadot
```bash
curl -o - -L http://snapshot.stakeworld.io/rocksdb-polkadot.lz4 | lz4 -c -d - | tar -x -C DBDIR
```
